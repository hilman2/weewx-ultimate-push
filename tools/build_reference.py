...
def main(argv=None):
    ...
    by_model = {}  # type: Dict[Tuple[str, int], List[Tuple[str, str]]]
    single = []
    ...
    for (model, limit), fields in sorted(by_model.items()):
        note = catalog.PLACEMENT_UNKNOWN.get(
            next(
                (
                    p
                    for p in catalog.CHANNELS
                    if catalog.CHANNELS[p][0] == model
                    and p in catalog.PLACEMENT_UNKNOWN
                ),
                '',
            ),
            '',
    ...
    lines.append(
        "Sensors with a single instance, and the readings a station sends "
    ...
    with open(args.out, 'w', encoding='utf-8', newline='\n') as fd:
        fd.write('\n'.join(lines) + '\n')
    print(
        "%d fields, %d sensors -> %s" % (len(catalog.FIELDS), len(by_model), args.out)
    )
    return 0
